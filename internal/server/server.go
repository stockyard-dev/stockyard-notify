package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-notify/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-notify"})})
    ...
    s.mux.HandleFunc("DELETE /api/templates/{id}",s.handleDelete)
    s.mux.HandleFunc("POST /api/templates/{id}/send",s.handleSend)
    s.mux.HandleFunc("GET /api/templates/{id}/sends",s.handleGetSends)
