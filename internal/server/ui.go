...
async function load(){var r=await fetch(A+'/alerts').then(function(r){return r.json()});items=r.alerts||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.recipient||'').toLowerCase().includes(q)||(i.channel||'').toLowerCase().includes(q)||(i.subject||'').toLowerCase().includes(q)||(i.body||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No items found.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1"><div class="item-title">'+esc(i.recipient)+'</div>';
if(i.channel)h+='<div class="item-sub">'+esc(i.channel)+'</div>';
...
if(i.error_msg)h+='<span>'+esc(i.error_msg)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/alerts/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{recipient:"",channel:"",subject:"",body:"",status:"",sent_at:"",error_msg:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' NOTIFY</h2>';
h+='<div class="fr"><label>Recipient *</label><input id="f-recipient" value="'+esc(i.recipient)+'"></div>';
...
h+='<div class="fr"><label>Error Msg</label><input id="f-error-msg" value="'+esc(i.error_msg)+'"></div>';
...
async function submit(){var body={recipient:document.getElementById('f-recipient').value.trim(),channel:document.getElementById('f-channel').value.trim(),subject:document.getElementById('f-subject').value.trim(),body:document.getElementById('f-body').value.trim(),status:document.getElementById('f-status').value.trim(),sent_at:document.getElementById('f-sent-at').value.trim(),error_msg:document.getElementById('f-error-msg').value.trim()};
if(editId){await fetch(A+'/alerts/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/alerts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
